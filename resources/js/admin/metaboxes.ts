...
(function ($) {
    function generateFormGroupHtml(
        type: CastCrewType,
        castcrewId: string | number,
        productionId: string | number,
        name: string = ''
    ): string {
        castcrewId = parseInt(castcrewId as string, 10);
        productionId = parseInt(productionId as string, 10);
    ...
    function addProductionCastCrew(type: CastCrewType): void {
        if (type !== CastCrewType.Cast && type !== CastCrewType.Crew) {
            throw new TypeError('Invalid value for type');
        }

        const productionId = $('#post_ID').val() as string;
        const castcrewId = $(`#ccwp-production-${type}-select`).val() as string;
     ...
     * @returns {void}
     */
    function disableThePostTitle(): void {
    ...
    function removeCastCrewFormGroup(this: JQuery | HTMLElement): void {
    ...
    $(function () {
        ...
        //    $castCrewSelectBox.each(function (index: number, ele: JQuery | HTMLElement) {
