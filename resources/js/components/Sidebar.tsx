...
const Sidebar: FC = () => {
    ...
    const attach = async () => {
        ...
        setError(null);
        try {
            await apiFetch({ path: `/ccwp/v1/relations`, method: 'POST', data: body, headers: { 'X-WP-Nonce': window.CCWP_SETTINGS?.nonce } } as any);
            // reload
            const query: any = {};
            if (isProduction) query.production_id = postId;
            if (isCastCrew) query.cast_and_crew_id = postId;
            const rows: any = await apiFetch({ path: `/ccwp/v1/relations?` + new URLSearchParams(query as any).toString(), headers: { 'X-WP-Nonce': window.CCWP_SETTINGS?.nonce } } as any);
            setRelations(rows || []);
            setNewTargetId('');
            setNewRole('');
            setNewOrder('');
    ...
    const detach = async (r: Relation) => {
        ...
        try {
            ...
            await apiFetch({ path: `/ccwp/v1/relations?${qs}`, method: 'DELETE', headers: { 'X-WP-Nonce': window.CCWP_SETTINGS?.nonce } } as any);
    ...
    useEffect(() => {
        ...
        apiFetch({ path: `/ccwp/v1/relations?` + new URLSearchParams(query as any).toString(), headers: { 'X-WP-Nonce': window.CCWP_SETTINGS?.nonce } } as any)
    ...
    return (
        <PluginSidebar name="ccwp-sidebar" title={title} icon={<Icon />}>
            <div>
