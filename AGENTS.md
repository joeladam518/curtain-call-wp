...
### PHP Code Quality
All composer commands must be run from the `plugin/` directory:
```bash
cd plugin
composer lint        # Lint PHP code using Mago
composer lint:fix    # Lint and auto-fix PHP code
composer format      # Auto-format PHP code using Mago
composer analyse     # Static analysis using Mago
composer analyse:fix # Static analysis with auto-fix
composer guard       # Run all Mago checks (lint + format + analyse)
composer build       # Install plugin deps optimized for production (--no-dev)
```
...
- Configuration: `plugin/mago.toml`
